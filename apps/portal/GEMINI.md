...
- **Qoder Wiki, Repowise, & Skills Maintenance**: When changing portal routes, APIs, auth, or startup procedures, update the corresponding documentation under `.agentic-tools-mcp/repowiki/en/content/Portal Application/` (symlinked at `.qoder/repowiki`), keep `.agentic-tools-mcp/repowise/` and `.agentic-tools-mcp/repowise-workspace/` synced (symlinked at `.repowise` and `.repowise-workspace`) by running `./tools/repowise/.venv/bin/repowise update -w --index-only`, and update custom skills (like `run-portal`) in `.qoder/skills/`. Use these directories/files as references.
