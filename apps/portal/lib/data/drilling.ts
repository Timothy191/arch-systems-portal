...
export async function getDrillingOpsData(deptId: string) {
  "use cache: remote";
  cacheTag(CACHE_TAGS.drillOperations, CACHE_TAGS.machines, CACHE_TAGS.employees);
  cacheLife("minutes");
...
export async function getMachineTelemetryData(
  ...
  "use cache: remote";
  cacheTag(CACHE_TAGS.machineTelemetry, CACHE_TAGS.drillOperations, CACHE_TAGS.machines);
  cacheLife("minutes");
  ...
  const machineNameMap = new Map((allMachines || []).map((m: { id: string; name: string }) => [m.id, m.name]));
