...
export class TelemetryController {
  ...
  async pushTelemetry(@Body() body: any) {
    // 1. Check if this is a Supabase Database Webhook payload
    const webhookResult = await this.telemetryService.processWebhookPayload(body);
  ...
  @ApiOperation({ summary: "Compute telemetry via Rust native plugin or JS fallback" })
  async rustTelemetry(@Body() body: { hours?: number; temp?: number; rpm?: number }) {
