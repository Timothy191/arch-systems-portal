...
export class AgentGraphService {
  ...
  async runAgentGraph(
    ...
    const nodeMap: Record<
      AgentNodeName,
      ((s: AgentState) => Promise<Partial<AgentState>>) | null
    > = {
  ...
  async finalizeAgentGraph(state: AgentState): Promise<void> {
    // pro-workflow integration: Explicit checkpointing of the final LLM state
    this.logger.debug(`[Pro-Workflow] Checkpointing state for session: ${state.sessionId}`);
  ...
  private async rateLimitNode(state: AgentState): Promise<Partial<AgentState>> {
    const result = await this.aiRateLimiterService.check(
      "chat",
      state.ip,
      30,
      60_000,
  ...
  private async resolveContextNode(
    state: AgentState,
  ): Promise<Partial<AgentState>> {
  ...
  private async loadMemoryNode(
    ...
    try {
      ...
      // Nucleoid Integration: Inject reasoning tags / relational markers into semantic context
      const enhancedContext = combined.map(mem => {
        const reasoningTag = mem.memoryType === "semantic" ? "[NUCLEOID_SYMBOLIC_FACT]" : "[NUCLEOID_EPISODIC_TRACE]";
        return `${reasoningTag} ${mem.content}`;
      });

      const memoryContext = enhancedContext.join("\\n");
  ...
  private async gatherContextNode(
    state: AgentState,
  ): Promise<Partial<AgentState>> {
    const latestUserMessage = [...state.messages]
      .reverse()
  ...
  private async executeToolsNode(
    ...
    for (const call of state.toolCalls) {
      try {
        // Per-tool rate limit
        const allowed = await this.aiRateLimiterService.check(
          "tool",
          state.ip,
          60,
          60_000,
        ...
        const result = await this.toolDispatchService.executeTool(
          state.userId,
          call.tool,
          call.args,
        );
  ...
  private async callLLMNode(state: AgentState): Promise<Partial<AgentState>> {
    const contextStr = [state.context ?? "", state.memoryContext ?? ""]
      .filter(Boolean)
      .join("\n\n");
