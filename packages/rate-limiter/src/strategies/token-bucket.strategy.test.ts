...
describe("TokenBucketStrategy", () => {
  ...
  describe("with RedisStore (Lua script)", () => {
    it("uses Lua script when eval is available", async () => {
      const mockResult = { allowed: 1, limit: 10, remaining: 9, reset_time: Date.now() + 60000 };
    ...
    it("blocks when Lua script returns allowed: 0", async () => {
      const now = Date.now();
      const mockResult = { allowed: 0, limit: 10, remaining: 0, reset_time: now + 60000 };
      mockRedisClient.eval.mockResolvedValue(JSON.stringify(mockResult));
  ...
  describe("distributed coordination", () => {
    it("handles concurrent requests with Redis", async () => {
      const mockResult = { allowed: 1, limit: 10, remaining: 8, reset_time: Date.now() + 60000 };
      mockRedisClient.eval.mockResolvedValue(JSON.stringify(mockResult));
    ...
    it("maintains consistency across distributed requests", async () => {
      const mockResult1 = { allowed: 1, limit: 10, remaining: 9, reset_time: Date.now() + 60000 };
      const mockResult2 = { allowed: 1, limit: 10, remaining: 8, reset_time: Date.now() + 60000 };
