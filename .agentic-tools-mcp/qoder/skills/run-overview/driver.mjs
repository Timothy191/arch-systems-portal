...
async function main() {
  console.log('Starting overview dev server (pnpm --filter arch-systems-overview dev)...');
  const dev = spawn('pnpm', ['--filter', 'arch-systems-overview', 'dev'], {
  ...
  const checks = [
    await checkEndpoint('/', {
      ...
      'contains app title': (_, body) => body.includes('Arch Systems Overview') || body.includes('overview'),
    }),
  ];
