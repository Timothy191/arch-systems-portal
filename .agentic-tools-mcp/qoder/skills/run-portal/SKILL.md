...
- `/` — root returns 307 redirect to `/login` (with Location header)
- `/login` — login page renders with form elements (200)
- `/api/health` — server responds (200 with Supabase, 500 in `--quick` mode without it)
...
Then open http://localhost:3000. The homepage redirects to `/login`.
