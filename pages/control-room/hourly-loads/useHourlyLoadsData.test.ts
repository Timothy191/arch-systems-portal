...
describe("useHourlyLoadsData", () => {
  ...
  beforeAll(() => {
    jest.useFakeTimers().setSystemTime(new Date("2026-07-08T12:00:00+00:00"));
  });
  ...
  it("returns correct data for both day and night shifts", () => {
    const m1 = "m1";
    const dayLoad = makeLoad({
      machine_id: m1,
      shift_type: "day",
      hour_01: 10,
    });
    const nightLoad = makeLoad({
      machine_id: m1,
      shift_type: "night",
      hour_01: 20,
    });
    const { result } = renderHook(() =>
      useHourlyLoadsData({
        machines: [{ id: m1, name: "M1", machine_type: "Dump Truck" }],
        hourlyLoads: [dayLoad, nightLoad],
      }),
    );

    expect(result.current.getHourValue(m1, 0)).toBe(10);

    act(() => {
      result.current.setSelectedShift("night");
    });

    expect(result.current.getHourValue(m1, 0)).toBe(20);
  });
});
