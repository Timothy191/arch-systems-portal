...
async function DrillingReportsContent({
  ...
  // Strict date validation: reject anything that isn't a real calendar day.
  // Without this, an attacker can pass `?from=0001-01-01&to=9999-12-31` to
  // force a full table scan (resource exhaustion) or `?from=__proto__` to
  // probe schema/ORM behavior. Matches the validation used by the generic
  // reports page (`[department]/reports/page.tsx`) so both surfaces agree.
  const dateRegex = /^\d{4}-\d{2}-\d{2}$/;
  const todayStr = new Date().toISOString().split("T")[0]!;
  const defaultFromStr = new Date(Date.now() - 30 * 86400000)
    .toISOString()
    .split("T")[0]!;

  const to =
    toParam && dateRegex.test(toParam) && toParam <= todayStr
      ? toParam
      : todayStr;
  const from =
    fromParam && dateRegex.test(fromParam) && fromParam < to
      ? fromParam
      : defaultFromStr;
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button variant="secondary" shape="rounded-lg" size="sm" asChild>
          <a
